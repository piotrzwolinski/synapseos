...
class GraphConnection:
    def __init__(self):
        ...
        self.graph_name = os.getenv("FALKORDB_GRAPH", "hvac")
        self._db = None
        self.graph = None
    ...
    def get_node_count(self):
        ...
        def _query():
            ...
            return result_value(result, "count")
        return self._execute_with_retry(_query)
    ...
    def get_graph_data(self):
        ...
        def _query():
            ...
            """)
            nodes = []
            for record in result_to_dicts(nodes_result):
                ...
                name = node_props.get("name") or node_props.get("title") or node_props.get("id") or f"Node {record.get('id', '?')}"
                nodes.append({
                    "id": str(record.get("id", "")),
                    "label": record["labels"][0] if record.get("labels") else "Node",
                    "name": str(name),
            ...
            """)
            relationships = []
            for record in result_to_dicts(rels_result):
                rel_props = record.get("properties", {})
    ...
    def fetch_graph_neighborhood(self, node_id: str, depth: int = 1, max_nodes: int = 50) -> dict:
        ...
        def _query():
            ...
            center_result = graph.query("""
                MATCH (center)
                WHERE id(center) = $node_id OR center.name = $node_id
                RETURN id(center) AS id, labels(center) AS labels, properties(center) AS properties
                LIMIT 1
            """, params={"node_id": node_id})

            center_record = result_single(center_result)
            ...
            neighbor_result = graph.query("""
                MATCH (center)-[r*1..""" + str(depth) + """]-(neighbor)
                WHERE id(center) = $center_id
                WITH DISTINCT neighbor, r
                LIMIT $max_nodes
                RETURN id(neighbor) AS id, labels(neighbor) AS labels, properties(neighbor) AS properties
            """, params={"center_id": center_record["id"], "max_nodes": max_nodes})

            nodes = [center_node]
            node_ids = {center_record["id"]}

            neighbor_count = 0
            for record in result_to_dicts(neighbor_result):
                neighbor_count += 1
                node_props = record.get("properties", {})
                if isinstance(node_props, dict):
                    node_props = dict(node_props)
                ...
                name = (node_props.get("name") or node_props.get("title") or
                       node_props.get("id") or f"Node {record.get('id', '?')}")
    ...
    def create_triggers_risk_relationship(self, concept_name: str, safety_risk_name: str) -> dict:
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                RETURN r
            """, params={"concept_name": concept_name, "safety_risk_name": safety_risk_name})
            return result_single(result) is not None
        return self._execute_with_retry(_query)
    ...
    def create_relationship(
        ...
        def _query():
            ...
            graph.query(f"""
                ...
                SET r += $properties
            """, params={"from_name": from_name, "to_name": to_name, "properties": props})
            return True
        return self._execute_with_retry(_query)
    ...
    def get_similar_cases(self, query_embedding: list[float], top_k: int = 3) -> list[dict]:
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                LIMIT $top_k
            """, params={"top_k": top_k * 2, "embedding": query_embedding})
            return result_to_dicts(result)
        return self._execute_with_retry(_query)
    ...
    def delete_project(self, project_name: str) -> dict:
        ...
        def _query():
            ...
            """, params={"name": project_name})
            counts = dict(result_single(count_result))
            ...
            cleanup_result = graph.query("""
                ...
                RETURN orphan_count
            """)
            orphan_record = result_single(cleanup_result)
        ...
        def _query():
            ...
            try:
                ...
                result = graph.query("""
                    ...
                    CALL db.idx.fulltext.queryNodes("project_fulltext", $search_term)
                ...
                """, params={"search_term": wildcard_term})
                return result_to_dicts(result)
            except Exception:
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                RETURN kc.raw_name AS raw_name, kc.type AS type,
                       ...
                       id(kc) AS id
            """, params={"raw_name": raw_name, "source_type": source_type, "inference_logic": inference_logic, "citation": citation, "event_name": event_name})
            return result_single(result)
        return self._execute_with_retry(_query)
        ...
        def _query():
            ...
            if action == "reject":
                ...
                result = graph.query("""
                    ...
                    RETURN vs.name AS source_name, id(vs) AS source_id
                """, params={"candidate_id": candidate_id, "verified_name": verified_name, "description": description or ""})
                record = result_single(result)
                return {
                    "action": "created",
                ...
                result = graph.query("""
                    ...
                    WHERE id(vs) = $existing_source_id
                    SET kc.status = 'verified'
                    MERGE (kc)-[:RESOLVED_TO]->(vs)
                    MERGE (vs)-[:ALIASED_AS {pattern: kc.raw_name}]->(kc)
                    RETURN vs.name AS source_name
                """, params={"candidate_id": candidate_id, "existing_source_id": existing_source_id})
                record = result_single(result)
                return {
                    "action": "mapped",
                    "candidate_id": candidate_id,
    ...
    def find_housings_by_duct_diameter(self, duct_diameter_mm: int) -> list[dict]:
        ...
        def _query():
            ...
            """, params={"diameter": duct_diameter_mm})
            return result_to_dicts(result)
        return self._execute_with_retry(_query)
    ...
    def search_product_variants(self, search_term: str) -> list[dict]:
        ...
        def _query():
            ...
            try:
                ...
                result = graph.query("""
                    CALL db.idx.fulltext.queryNodes("product_variant_fulltext", $term)
            ...
            for record in result_to_dicts(result):
                record["categories"] = [c for c in record.get("categories", []) if c.get("type")]
        ...
        def _query():
            ...
            try:
                pv_result = graph.query("""
                    ...
                    LIMIT 5
                """, params={"term": wildcard_term})
                results["variants"] = result_to_dicts(pv_result)
            except Exception:
                # Fallback to old query if fulltext index doesn't exist
                pv_result = graph.query("""
                    MATCH (pv:ProductVariant)
            ...
            try:
                fc_result = graph.query("""
                    CALL db.idx.fulltext.queryNodes("filter_cartridge_fulltext", $term)
                    ...
                    LIMIT 5
                """, params={"term": wildcard_term})
                results["cartridges"] = result_to_dicts(fc_result)
            except Exception:
                fc_result = graph.query("""
            ...
            try:
                fcons_result = graph.query("""
                    CALL db.idx.fulltext.queryNodes("filter_consumable_fulltext", $term)
                    ...
                    LIMIT 5
                """, params={"term": wildcard_term})
                results["filters"] = result_to_dicts(fcons_result)
            except Exception:
                fcons_result = graph.query("""
                    MATCH (f:FilterConsumable)
            ...
            try:
                mat_result = graph.query("""
                    CALL db.idx.fulltext.queryNodes("material_spec_fulltext", $term)
                    ...
                    LIMIT 5
                """, params={"term": wildcard_term})
                results["materials"] = result_to_dicts(mat_result)
            except Exception:
                mat_result = graph.query("""
    ...
    def ensure_learned_rules_index(self) -> bool:
        ...
        def _query():
            graph = self.connect()
            # Create vector index (idempotent — catch "already exists")
            try:
                graph.query(f"""
                    CREATE VECTOR INDEX FOR (k:Keyword) ON (k.embedding)
                    OPTIONS {{dimension: {VECTOR_DIMENSIONS}, similarityFunction: 'cosine'}}
                """)
            except Exception as e:
                if "already exists" not in str(e).lower():
                    raise
    ...
    def save_learned_rule(self, trigger_text: str, rule_text: str,
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                RETURN
                    id(k) AS keyword_id,
                    id(r) AS requirement_id,
                    k.name AS keyword,
                    r.text AS requirement,
                    rel.confidence AS confidence
            """, params={"trigger": trigger_text, "rule": rule_text, "embedding": embedding, "context": context, "confirmed_by": confirmed_by})
    ...
    def delete_learned_rule(self, trigger: str, rule: str) -> bool:
        ...
        def _query():
            ...
            """, params={"trigger": trigger, "rule": rule})
            return result_single(result) is not None
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                RETURN app.id AS id,
                       ...
                       [r IN requirements WHERE r.id IS NOT NULL] AS requirements
                LIMIT 1
            """, params={"keywords": [k.lower() for k in keywords]})
            return result_single(result)
        ...
        def _query():
            ...
            result = graph.query("""
                ...
                RETURN app.id AS id,
                       ...
                       [r IN requirements WHERE r.id IS NOT NULL] AS requirements
                ORDER BY score DESC
            """, params={"embedding": query_embedding, "top_k": top_k, "min_score": min_score})
            return result_to_dicts(result)
    ...
    def get_connection_length_offset(self, family_id: str, connection_code: str) -> int:
        ...
        def _query():
            ...
            """, params={"fid": family_id, "code": connection_code})
            record = result_single(result)
            ...
            if product_family:
                ...
                result = graph.query("""
                    ...
                    RETURN pv.reference_airflow_m3h AS reference_airflow_m3h,
                           ...
                           pv.height_mm AS height_mm
                    LIMIT 1
                """, params={"pf_id": pf_id, "w": width_mm, "h": height_mm})
            else:
                result = graph.query("""
                    MATCH (pv:ProductVariant)
                    WHERE ((pv.width_mm = $w AND pv.height_mm = $h)
            ...
            if product_family:
                result = graph.query("""
                    ...
                    RETURN DISTINCT
                           ...
                           q.priority AS priority
                    ORDER BY q.priority
                """, params={"app_id": application_id, "family": product_family})
            else:
                result = graph.query("""
                    MATCH (rule:ClarificationRule)-[:TRIGGERED_BY_CONTEXT]->(app:Application)
                    WHERE app.id = $app_id OR app.name = $app_id
    ...
    def check_material_suitability(self, application_name: str, material_code: str) -> dict:
        ...
        def _query():
            ...
            """, params={"app_name": application_name, "material_code": material_code})
            record = result_single(result)
            if record:
                return {
    ...
    def get_accessory_compatibility(self, accessory_code: str, product_family: str) -> dict:
        ...
        def _query():
            ...
            """, params={"acc_code": accessory_code, "family": product_family})
            record = result_single(result)
    ...
    def check_unmitigated_physics_risks(self, product_family: str, environment_id: str = None, environment_keywords: list[str] = None) -> list[dict]:
        ...
        def _query():
            ...
            """, params={"product_family": product_family, "env_id": environment_id, "keywords": environment_keywords or []})
    ...
    def get_graph_reasoning_context(self, application_name: str, product_family: str, material_code: str = None) -> dict:
        ...
        def _query():
            ...
            """, params={"app_name": application_name, "family": product_family, "material": material_code})
            record = result_single(result)
    ...
    def get_product_family_data_dump(self, product_family: str) -> dict:
        ...
        def _query():
            ...
            """, params={"pf_id": pf_id})
            variants = result_to_dicts(variants_result)

            # Query 2: Get all materials
            materials_result = graph.query("""
                MATCH (ms:MaterialSpecification)
                RETURN ms.code AS code,
            ...
            """)
            materials = result_to_dicts(materials_result)

            # Query 3: Get material restrictions by application/environment
            restrictions_result = graph.query("""
            ...
            """)
            restrictions = result_to_dicts(restrictions_result)

            # Query 4: Get product vulnerabilities
            vulnerabilities_result = graph.query("""
            ...
            """, params={"family": product_family})
            vulnerabilities = result_to_dicts(vulnerabilities_result)

            # Query 5: Get insulation requirements (for outdoor/condensation risks)
            insulation_result = graph.query("""
            ...
            """)
            insulation_rules = result_to_dicts(insulation_result)
            ...
            """, params={"pf_id": pf_id})
            pf_meta = result_single(pf_result) or {}
    ...
    def init_session_schema(self):
        ...
        schema_statements = [
            # FalkorDB: no named indexes, no IF NOT EXISTS — use try/except
            "CREATE INDEX FOR (s:Session) ON (s.id)",
            "CREATE INDEX FOR (ap:ActiveProject) ON (ap.id)",
    ...
    def get_expert_conversations(self, limit: int = 50, offset: int = 0) -> dict:
        ...
        graph = self.connect()
        # Count total
        total_result = graph.query("""
            MATCH (p:ActiveProject)
            WHERE (p)-[:HAS_TURN]->(:ConversationTurn)
            RETURN count(p) AS total
        """)
        total = result_value(total_result, "total")
        ...
        conversations = result_to_dicts(result)
    ...
    def get_conversation_detail(self, session_id: str) -> dict:
        ...
        proj = result_single(proj_result) or {
        ...
        """, params={"sid": session_id})
        reviews = result_to_dicts(reviews_result)
    ...
    def save_judge_results(self, session_id: str, turn_number: int,
        ...
        result = graph.query("""
            ...
            RETURN ct.id AS id
        """, params={"sid": session_id, "tn": turn_number, "jr": judge_results})
        return result_single(result) is not None
    ...
    def submit_expert_review(self, session_id: str, reviewer: str,
        ...
        """, params={"session_id": session_id, "review_id": review_id, "reviewer": reviewer, "comment": comment, "overall_score": overall_score, "dimension_scores": dimension_scores, "provider": provider, "turn_number": turn_number})
        record = result_single(result)
    ...
    def get_expert_reviews_summary(self) -> dict:
        ...
        """)
        stats = result_single(stats_result) or {
            "total": 0, "positive": 0, "negative": 0
        }

        recent_result = graph.query("""
            MATCH (er:ExpertReview)
            ...
            LIMIT 20
        """)
        recent = result_to_dicts(recent_result)
    ...
    def get_environment_keywords(self) -> dict[str, list[str]]:
        ...
        """)
        return {r["env_id"]: r["keywords"] for r in result_to_dicts(result)}
    ...
    def get_application_properties(self, app_id: str) -> dict:
        ...
        """, {"app_id": full_id})
        return result_single(result) or {}
    ...
    def get_size_determined_properties(
        ...
        if module_id.startswith("PV_"):
            ...
            """, {"module_id": module_id})
            pv_props = result_to_dicts(pv_result)
            if pv_props:
                return pv_props
