...
def apply_polis_constraint():
    ...
    with driver.session(database=db.database) as session:
        ...
        if record:
            print(f"[OK] Created/updated Polis option: {record['id']} with min_length={record['min_length']}mm")
        ...
        for record in result:
            print(f"[OK] Linked Polis to {record['family']} -> {record['feature']}")
        ...
        """)
        incompatible = list(result)
        if incompatible:
            for record in incompatible:
                print(f"[OK] Set incompatibility: {record['opt']} <-> {record['variant']}mm variant")
        else:
