...
function PromptViewer({ title, content, description }: { title: string; content: string; description: string }) {
  ...
  return (
    <div className="rounded-lg border border-green-100 dark:border-green-800 bg-green-50/30 dark:bg-green-950/20 overflow-hidden">
      ...
      <div className="p-3">
        <pre className={`text-xs text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-mono bg-white dark:bg-slate-800 p-3 rounded border border-slate-100 dark:border-slate-700 ${expanded ? "max-h-96" : "max-h-32"} overflow-y-auto`}>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-200">Domain Configuration</h1>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 text-sm font-medium transition-colors"
          >
                ...
                {domains?.available_domains.map((domain) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3">
                        <h3 className="font-semibold text-slate-800 dark:text-slate-200">{domain.company}</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{domain.name}</p>
                  ...
                  <CardContent className="pt-0">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                        ...
                        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-1">{config.domain.company}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                        <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wide">Domain ID</p>
                        <p className="text-sm font-mono text-slate-800 dark:text-slate-200 mt-1">{config.domain.id}</p>
                      </div>
                      <div className="col-span-2 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                        <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wide">Description</p>
                        <p className="text-sm text-slate-800 dark:text-slate-200 mt-1">{config.domain.description}</p>
                      </div>
                    </div>
                  </CardContent>
                )}
                  ...
                  <CardContent className="pt-0">
                    <div className="grid grid-cols-4 gap-3 mb-4">
                      ...
                      <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800">
                        ...
                        <p className="text-xs text-amber-600 dark:text-amber-400">Environments</p>
                      </div>
                      <div className="p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800">
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-3">
                      {config.demanding_environments.map((env) => (
                        <div key={env.name} className="p-3 rounded-lg border border-amber-100 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20">
                          <div className="flex items-center justify-between mb-2">
                            <span className="font-medium text-slate-800 dark:text-slate-200 capitalize">{env.name.replace(/_/g, " ")}</span>
                            <div className="flex gap-1">
                              {env.required_materials.map((mat) => (
                                <Badge key={mat} className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">{mat}</Badge>
                              ))}
                            </div>
                          </div>
                          <p className="text-sm text-slate-600 dark:text-slate-400">{env.concern}</p>
                        </div>
                      ))}
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-3">
                      {config.product_capabilities.map((prod) => (
                        <div key={prod.family} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                          <div className="flex items-center justify-between mb-2">
                            <div>
                              <span className="font-medium text-slate-800 dark:text-slate-200">{prod.family}</span>
                              <span className="text-slate-400 ml-2">-</span>
                              <span className="text-sm text-slate-600 dark:text-slate-400 ml-2">{prod.name}</span>
                            </div>
                        ...
                        <div key={key}>
                          ...
                          <div className="space-y-1 pl-7">
                            {category.questions.slice(0, 3).map((q, i) => (
                              <p key={i} className="text-sm text-slate-600 dark:text-slate-400 truncate">{q}</p>
                            ))}
                  ...
                  {expandedSections.has("templates") && (
                    <CardContent className="pt-0 space-y-3">
                      {Object.entries(config.prompt_templates).map(([key, template]) => (
                        <div key={key} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700">
                          <div className="flex items-center justify-between mb-2">
                          ...
                          <pre className="text-xs text-slate-600 dark:text-slate-400 whitespace-pre-wrap font-mono bg-white dark:bg-slate-800 p-2 rounded border border-slate-100 dark:border-slate-700 max-h-24 overflow-y-auto">
                            {template}
                          </pre>
                        </div>
                      ))}
